...
بهتر است به جای این تابع از تابع createLoginUrl که ساده‌تر است و کارآمدی بیشتری دارد استفاده شود. با استفاده از این تابع می‌توان برای یک کاربر خاص که پیش‌تر ساخته شده و دسترسی آن به اتاق مورد نظر داده شده است لینک ورود مستقیم ایجاد کرد تا کاربر بدون نیاز به درج نام کاربری و گذرواژه وارد اتاق شود.
##### درخواست:
```json
{
  "action": "getLoginUrl",
...
پارامتر `user_id` اختیاری است و می‌تواند شامل عدد یا رشته (بدون فاصله) باشد و هدف از آن جلوگیری از ورود همزمان چند کاربر با استفاده از لینک تولید شده می‌باشد. به بیان دیگر اگر این پارامتر فاقد مقدار باشد، پارامتر `concurrent` هیچ تاثیری ندارد و لینک تولید شده عمومی خواهد بود و با استفاده از این لینک به تعداد نامحدود کاربر می‌توانند به صورت همزمان وارد اتاق شوند.
